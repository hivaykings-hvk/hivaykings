...
const CompareRatingsSection: React.FC<CompareRatingsSectionProps> = ({ chiefRating, communityRating }) => {
    ...
    return (
        <div className="flex justify-center bg-gray-50">
            <section className="mt-12 px-4 py-12">
                <h2 className="mb-8 text-center text-3xl font-bold">Compare Ratings</h2>
                <div className="flex flex-col justify-center gap-8 lg:flex-row">
                    {/* Chief's Rating Card */}
                    <div className="w-full max-w-md rounded-lg bg-yellow-50 p-6 shadow-md lg:w-1/2">
                        <div className="mb-6 flex items-center">
                            <div className="mr-4 h-12 w-12 overflow-hidden rounded-full">
                                <Avatar className="h-full w-full bg-gray-100">
                                    <AvatarImage src={getImageUrl(chiefRating?.chiefUser?.image)} className="object-cover" />
                                    <AvatarFallback className="text-gray-800">
                                        {`${chiefRating?.chiefUser?.firstName?.[0]}${chiefRating?.chiefUser?.lastName?.[0]}`}
                                    </AvatarFallback>
                                </Avatar>
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-gray-800">{`Chief's Rating`}</h3>
                        ...
                        <div className="space-y-2">
                            <RatingItem label="Road Condition" rating={chiefRating.roadCondition} color="text-yellow-500" />
                            <RatingItem label="Traffic" rating={chiefRating.traffic} color="text-yellow-500" />
                            <RatingItem label="Facilities" rating={chiefRating.facilities} color="text-yellow-500" />
                            <RatingItem label="Safety Index" rating={chiefRating.safetyIndex} color="text-yellow-500" />
                            <RatingItem label="Scenic Value" rating={chiefRating.scenicValue} color="text-yellow-500" />
                        </div>
                    </div>

                    {/* Community Ratings Card */}
                    <div className="w-full max-w-md rounded-lg bg-blue-50 p-6 shadow-md lg:w-1/2">
                        <div className="mb-6 flex items-center">
