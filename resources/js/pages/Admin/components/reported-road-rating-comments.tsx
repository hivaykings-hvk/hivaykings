...
export function ReportedRoadRatingCommentsForm() {
    const [comments, setComments] = useState<RoadRatingComment[]>([]);
    ...
    const fetchComments = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/admin/reported-road-rating-comments?per_page=${perPage}&page=${currentPage}`);
    ...
    const handleDeleteComment = async (commentId: string) => {
        try {
            ...
            const response = await fetch(`/api/admin/road-rating-comments/${commentId}`, {
                method: 'DELETE',
                headers: {
    ...
    const handleDiscardComment = async (commentId: string) => {
        try {
            setDeleting(true);
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
            const response = await fetch(`/api/admin/road-rating-comments/${commentId}/discard`, {
                method: 'PATCH',
                headers: {
                    'X-CSRF-Token': csrfToken,
                    'X-Requested-With': 'XMLHttpRequest',
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Reported Road Rating Comments</h1>
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Manage all reported road rating comments in one place</p>
            </div>
                                        ...
                                        <tr key={comment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            <td className="px-6 py-4">
                                                <p className="text-sm text-slate-600 dark:text-slate-400">
                                                    {comment.road_rating
                                                        ? `${comment.road_rating.from_city} → ${comment.road_rating.to_city}`
                                                        : 'Unknown Route'}
                                                </p>
                                            </td>
                                            <td className="px-6 py-4">
