...
export function ReportedQuestionCommentsForm() {
    ...
    const fetchComments = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/admin/reported-question-comments?per_page=${perPage}&page=${currentPage}`);
    ...
    const handleDeleteComment = async (commentId: string) => {
        try {
            ...
            const response = await fetch(`/api/admin/question-comments/${commentId}`, {
                method: 'DELETE',
                headers: {
    ...
    const handleDiscardComment = async (commentId: string) => {
        try {
            setDeleting(true);
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
            const response = await fetch(`/api/admin/question-comments/${commentId}/discard`, {
                method: 'PATCH',
                headers: {
                    'X-CSRF-Token': csrfToken,
                    'X-Requested-With': 'XMLHttpRequest',
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Reported Question Replies</h1>
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Manage all reported question replies in one place</p>
            </div>
